...
import {
  createSupplierAndSendMagicLinkAction,
  type SupplierSignupActionState,
} from "./actions";

const INITIAL_STATE: SupplierSignupActionState = {
  success: false,
  error: null,
  fieldErrors: {},
};
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        required
        disabled={disableFields}
        error={fieldErrors.primary_process}
        placeholder="CNC milling, sheet metal, 3D printing..."
      />
      ...
      {state.success ? (
        <div className="rounded-2xl border border-blue-400/40 bg-blue-400/10 px-4 py-4 text-sm text-blue-100">
          Magic link sent to{" "}
          <span className="font-mono text-white">
            {state.submittedEmail ?? "your inbox"}
          </span>{" "}
          – open it on this device to finish onboarding.
        </div>
      ) : null}
...
function SubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <button
      ...
      disabled={isDisabled}
    >
      {pending ? "Sending..." : "Create supplier profile & send magic link"}
    </button>
  );
}
