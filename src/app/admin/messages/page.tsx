...
function threadPillClasses(label: string) {
...
export default async function AdminMessagesPage() {
  ...
  const sorted = rows.slice().sort((a, b) => {
    const aPinned = a.needsReplyFrom === "admin" || a.unreadCount > 0;
    const bPinned = b.needsReplyFrom === "admin" || b.unreadCount > 0;
  ...
  return (
    <AdminDashboardShell
      title="Messages"
      description="Triage every RFQ conversation with a consistent “needs reply” signal."
      actions={
        <Link
          href="/admin/quotes"
    ...
    >
      {sorted.length === 0 ? (
        <div className="rounded-2xl border border-slate-900/70 bg-black/40 p-6 text-sm text-slate-300">
          No conversations yet. Messages will appear here once quotes have activity.
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-slate-900/70 bg-black/40">
          <table className="min-w-full divide-y divide-slate-900/70 text-sm">
            ...
            <tbody className="divide-y divide-slate-900/70">
              {sorted.map((row) => {
                const threadLabel = resolveThreadStatusLabel("admin", row.needsReplyFrom);
                ...
                return (
                  <tr key={row.quoteId} className="hover:bg-slate-900/50">
                    <td className="px-5 py-4 align-middle">
                      <Link
                        href={`/admin/quotes/${row.quoteId}#messages`}
