...
export default function SupplierTakeAction(props: {
  ...
  const statusAction = useMemo(() => {
    ...
    const label = status === "paused" ? "Unpause supplier" : "Pause supplier";
    return { next, label };
  }, [status]);
    ...
    <div className="flex flex-wrap items-center gap-3">
      {threadHref ? (
        ...
        <span
          ...
          className="inline-flex cursor-not-allowed items-center justify-center rounded-xl border border-slate-900 bg-slate-950/30 px-4 py-2 text-sm font-semibold text-slate-500"
        >
          Open thread
        </span>
      )}
      ...
      {props.supportsQuoteMessages ? (
        <button
          ...
          className="inline-flex items-center justify-center rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-2 text-sm font-semibold text-slate-100 transition hover:border-emerald-500/40 hover:text-emerald-200 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {pending === "nudge" ? "Nudging…" : "Nudge supplier"}
        </button>
      ) : null}
