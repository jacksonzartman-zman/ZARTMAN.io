...
export default async function AdminNotificationsPage() {
  ...
  async function markAllRead() {
    "use server";
    const unread = await loadUserNotifications(admin.id, { onlyUnread: true, limit: 500 });
    await markNotificationsRead(admin.id, unread.map((n) => n.id));
  }
  ...
  async function markOneRead(formData: FormData) {
    ...
    if (!id) return;
    await markNotificationsRead(admin.id, [id]);
  }
  ...
  return (
    <AdminDashboardShell
      title="Notifications"
      description="Actionable items across RFQs, messages, kickoff, capacity, and system health."
      actions={
