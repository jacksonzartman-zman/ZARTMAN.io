...
export default function PortalSearchInput({
  initialValue = "",
  placeholder = "Search...",
  basePath,
  inputType = "search",
  ...
  const navigateWithValue = (nextValue: string) => {
    ...
    // Fallback behavior (legacy key). Prefer listStateConfig wherever possible.
    ...
    >
      <div className="relative flex-1">
        ...
        <input
          ...
          className="w-full rounded-lg border border-slate-700 bg-slate-950/60 py-2 pl-9 pr-10 text-sm text-slate-100 placeholder:text-slate-500 outline-none transition focus:border-emerald-400 focus:bg-slate-950"
          disabled={isPending}
        />
        {value.length > 0 && (
          <button
